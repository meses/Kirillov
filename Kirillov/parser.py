...
def get_page_url(html):
    ...
    ads = soup.find('div', class_='catalog-list').find_all('div', class_='item_table') # Всего объявлений на странице. ad - обявление; ads - список объявлений
    ...
    for ad in ads:
        ...
        try:
            ...
            price = ''
        try:
            metro = ad.find('div', class_='data').find_all('p')[-1].text.strip() # Показывает станцию метро
        except:
            metro = ''
